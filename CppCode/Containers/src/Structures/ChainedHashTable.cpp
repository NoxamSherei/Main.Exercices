...
namespace DataStructures {
	void ChainedHashTable::Insert(std::string input)
	{
	...
	const bool ChainedHashTable::Contain(std::string input)
	{
		if (IsEmpty()) {
			return false;
		}
	...
	const size_t ChainedHashTable::Depth() const
	{
		if (IsEmpty()) {
			return 0;
		}
	...
	const size_t ChainedHashTable::Count() const
	{
		return size;
	}

	const bool ChainedHashTable::IsEmpty() const
	{
		return size == 0;
	}
	...
	int ChainedHashTable::GetIdFromHash(unsigned int hash) {
